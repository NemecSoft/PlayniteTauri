...
export default function AdminApp() {
  ...
  const saveGame = async () => {
    ...
    const payload: Game = {
      ...
      // 显式带上这些后端字段，避免依赖 spread 推断导致 serde 报缺字段。
      playTask: editingGame?.playTask ?? null,
      otherTasks: editingGame?.otherTasks ?? [],
      lastPlayed: editingGame?.lastPlayed ?? null,
      playCount: editingGame?.playCount ?? 0,
      lastActivity: editingGame?.lastActivity ?? null,
      playtime: editingGame?.playtime ?? 0,
      added: editingGame?.added ?? base.added,
